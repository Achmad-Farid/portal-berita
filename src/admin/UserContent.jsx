  ...
  return (
    <div className="bg-neutral-light dark:bg-neutral-dark text-neutral-dark dark:text-text-dark p-6">
      ...
      <div className="flex items-center mb-4 space-x-4">
        ...
        />
        <select className="border p-2 rounded bg-neutral-light dark:bg-neutral-700 text-neutral-dark dark:text-text-dark focus:outline-none" value={role} onChange={handleRoleChange}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {statusFetchUsers === "loading" || statusFetchUsersByRole === "loading" ? (
          ...
          users
            ...
            .map((user, i) => (
              <div key={i} className="border p-4 rounded shadow bg-white dark:bg-neutral-700 dark:border-neutral-600 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-neutral-dark dark:text-text-dark">{user.username}</h3>
                ...
                <div className="space-x-2 flex flex-col items-center gap-2">
                  <select value={user.role} onChange={(e) => handleEditChange(user._id, e.target.value)} className="px-2 py-1 bg-secondary text-white rounded dark:bg-secondary dark:hover:bg-secondary-dark">
                    <option value="user">User</option>
                    <option value="journalist">Journalist</option>
                  </select>

                  <button onClick={() => handleDelete(user._id)} className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-400 dark:hover:bg-red-600">
                    Delete
                  </button>
                </div>
              </div>
